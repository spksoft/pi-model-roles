...
| Coarse privacy scope | **User-wide policy:** `/model-roles context prompt` saves a durable opt-out; other sessions require `/reload`. New configs use bounded conversation, while existing files omitting the field remain prompt-only. There is no session-scoped override or automatic secret redaction; visible text and unmarked templates may remain. |
