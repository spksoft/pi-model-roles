...
| Registered prompt templates (for example `/plan <description>`) and `/skill:name <task>`, submitted interactively while idle | Select once from raw command/arguments before Pi expands them, plus the selected bounded retained-context mode if enabled. The current expansion is not read for selection. |
...
- **Data and cost:** at most one prompt-only selector request from the raw target command and arguments, even when conversation or full mode is enabled, subject to the normal limits and fallback rules. The wrapper does not read referenced files, command descriptions, generated kickoff prompts, or plan contents for classification. A filename may be insufficient to distinguish task roles; use `/model-roles use <role>` for an exact choice. Arguments are not secret-filtered. The wrapper is text-only and does not forward new image attachments; existing conversation image-capability checks still apply.
...
The experimental per-turn context hook and unbounded full-history projector remain withdrawn. Routing stays at the supported idle primary-TUI `input` boundary, before model/authentication checks and pre-prompt compaction. Opt-in `conversation` projects bounded retained dialogue, while separately consented `full` adds newest retained tool calls/results up to 100,000 UTF-8 bytes through public session APIs at that same input boundary; neither is final-provider-context routing. Registered prompt-template and skill submissions route once from raw command text plus that optional context; expansion itself does not trigger another selection. Tool loops, queued work, extension-owned commands, Auto Setup, non-TUI modes, and children are not independently rerouted. No dependency is patched.
...
| A task used default instead of a custom role | Inspect `/model-roles why` for `no_match`, `ambiguous`, unavailable-role warnings, or selector failure. Make descriptions distinct. For follow-ups such as “implement that”, new configs use `conversation`; existing files omitting `selectorContext` can enable it with `/model-roles context conversation`. Use separately consented `full` only when newest tool calls/results are needed. Incomplete context still falls back. |
...
| History or tool-output inclusion is unwanted | Run `/model-roles context prompt` to save prompt-only selection; other sessions need `/reload`. Already transmitted requests cannot be undone. |
...
Task text beyond 65,536 Unicode characters, blank/image-only input, or an insufficient selector context budget skips classification and uses fallback. See [all limits](configuration.md#advanced-limits-and-safeguards).
...
The saved selector context applies user-wide; other sessions need `/reload` to pick up changes. No compaction, final-payload inspection, background child routing, mid-task routing, or atomic cancellation of in-flight Pi `setModel` has been added.
