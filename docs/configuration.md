...
3. The normal agent should research first-party material for every exact selected provider/model ID and submit a structured report. The prompt asks it to compare each selected model at every supported effort (including a single selected model), then propose as many distinct, useful whole-task roles as the evidence and selector boundaries justify. The same model can serve several roles at different efforts; it does not require one role per model/effort pair. Criteria must be decidable from the submitted task and any context exposed by the configured selector policy, avoid execution instructions and model claims in descriptions, and pass a pairwise overlap check. Good/bad few-shot descriptions demonstrate these boundaries. It may instead report that no official evidence was found, that it used offline knowledge, or that a serving-model identity could not be resolved. A source URL is agent-reported and user-reviewable, not independently verified by this package. Offline knowledge means no web evidence was collected; it is not a local model run.
...
- **Useful coverage without duplicate roles.** Explore the selected models × their supported efforts, including multiple effort choices for one selected model. Cover distinct observable task families when a pair has a justified advantage, rather than aiming for a fixed role count. OMP's task roles are inspiration, not a template: its explicit plan/subagent/commit modes and non-chat subsystems cannot be inferred or configured by this whole-task selector. Check proposals against retained existing roles; reuse an existing ID for an intentional replacement and describe it in the summary. If a partial merge keeps old boundaries, review those for overlap yourself—the package does not semantically validate descriptions.
- **Only observable context.** The selector cannot see actual repository size, remaining context or image contents. Prompt mode sees no history; `conversation` supplies bounded retained text/summaries, while separately consented `full` can include the newest retained tool calls/results. Neither mode is Pi's final execution payload or an unbounded transcript. Auto Setup cannot opt you into history sharing. A short submitted task can still need substantial execution context. Roles cover whole tasks, not automatic plan/code/review phases. A larger context window is a capacity limit, not a quality score.
- **Effort needs justification.** Choose the lowest supported effort justified by each role's reliability needs, with extra effort justified relative to lower supported efforts on the same model by uncertainty, risk or competing constraints. Supported does not mean optimal; label unmeasured choices provisional. Effort labels are not comparable compute budgets across providers. Default recommendations must consider selector overhead as well as general-task reliability; `enabled` and the selector deadline remain unchanged.
...
```yaml
version: 1
selectorContext: conversation # use full only after its separate tool-output disclosure
roles:
  default:
    model: inherit
    effort: inherit
...
`full` mode requires its own confirmation. It keeps the newest retained records, with up to **100 messages** and **100,000 UTF-8 text bytes** total. It includes user/assistant text, compaction/branch summaries, raw assistant tool-call names/arguments, tool-result text, and ordinary `!` shell command/output text. When a record or the total exceeds the cap, it retains the newest complete Unicode text. Selector-budget checks may still remove oldest full-context records; they never shorten the submitted task or role descriptions.

Both modes exclude raw thinking/reasoning blocks, image bytes, custom entries/messages, and separately loaded system/repository/context files. Full mode also honors `!!` shell commands' existing exclusion from normal LLM context, but can include retained expanded skill text; conversation mode reduces recognized skill envelopes to `/skill:name` and arguments. **This is not secret redaction:** retained visible dialogue, summaries, tool arguments/results, and unmarked template expansions can contain secrets or prompt injection. They are sent to the configured selector provider (default-role provider unless overridden), potentially a different provider from execution, under that provider's retention/pricing policy. The current submitted task is still sent intact, including any pasted sensitive content. Neither mode adds a summarizer, new tool call, or changes Pi compaction.
...
Unavailable, unauthenticated, or out-of-scope models remain in your YAML and appear with warnings. They are not eligible candidates; the menu checks availability again before saving a chosen model. Image attachments or images retained in either context mode prevent selection of text-only execution models. The selector itself receives no image bytes.
...
| Task text: 65,536 Unicode characters | Larger tasks skip classification and use fallback. Blank/image-only tasks also use default. |
| Selector context budget | UTF-8 bytes of its system/data payload, plus 2,048 output tokens and 1,024 overhead, must fit the selector context window. Either context mode drops oldest optional records first; if task/roles still cannot fit, classification is skipped. |
| Conversation projection | `conversation`: at most 12 messages, 4 KiB UTF-8 each, 16 KiB total. `full`: at most 100 retained records and 100,000 UTF-8 text bytes, newest text first; tool calls/results are included only after explicit full-mode consent. |
| Selector output: 2,048 tokens requested; 16,384 UTF-8 bytes accepted | Prompt mode requires exact JSON such as `{"matches":["quick"]}`. Conversation and full modes require `{"action":"classify","matches":["quick"]}` or `{"action":"continue","matches":[]}`. Extra fields, duplicate/unknown IDs, nonempty continuation matches, code fences, tool output, malformed or incomplete/error responses fall back. |

Selector data contains sorted eligible role IDs/descriptions and submitted task text; `conversation` or separately consented `full` mode adds the matching bounded projection and previous role identifier, never new execution authority. One clear match selects a role; zero or multiple matches use default unless a valid contextual continuation is requested. There is at most one request per eligible idle primary-TUI submission and no retry. Oversized task text is skipped intact, not truncated or summarized.
...
Prompt mode does not collect conversation/reasoning/tool history, expanded template/skill bodies, command descriptions/source paths, system prompts, or context files for classification. Conversation mode adds only its bounded dialogue/summary projection; full mode adds the separately consented newest retained tool calls/results described above. Neither mode inspects referenced files separately. Text pasted into a submitted task (or supplied by an earlier input-transforming extension) is still task text and may be sent; there is no reliable secret-redaction guarantee. Auto Setup research/refinement stays on its original model/effort without independent routing. Per-turn and automatic-child routing are [unsupported on the tested host](compatibility.md#per-turn-routing-is-not-supported); there is no YAML flag that enables them.
