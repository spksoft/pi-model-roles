...
2. Read and confirm the disclosure. Research uses the **current Pi model** without switching it. The generated prompt includes selected candidates' exact IDs, supported efforts, image support and context windows, plus the loaded configuration snapshot's role descriptions/assignments, `enabled`, `selectorTimeoutMs`, and effective `selectorContext`. The model also receives its normal Pi conversation/provider flow and may use the tools you already configured. Do not put secrets in descriptions. Tool providers can make requests, charge, or retain data under their own policies. Auto Setup does not sandbox them.
...
This is user-wide, not per-project. Successful changes take effect in the current session without enabling routing or clearing a manual pause; other sessions need `/reload`. Cancellation or failed persistence leaves the previous policy active. Editing YAML directly bypasses the confirmation dialog and is an explicit opt-in. Auto Setup never changes this policy.
...
Excluded: raw thinking/reasoning blocks, tool calls/results, image bytes, arbitrary custom entry data, and separately loaded system/repository/context files. Recognized Pi skill envelopes are reduced to `/skill:name` and arguments. **This is not secret redaction:** retained visible dialogue and summaries can contain secrets, copied tool/resource content, or unmarked past template expansions. They are sent to the default selector provider, potentially a different provider from execution, under that provider's retention/pricing policy. The current submitted task is still sent intact, including any pasted sensitive content.
