...
| Selector output: 2,048 tokens requested; 16,384 UTF-8 bytes accepted | Output must be exact JSON such as `{"matches":["quick"]}`. Extra JSON fields, duplicate or unknown role IDs, code fences, tool output, malformed or incomplete/error responses fall back. |
...
The package does not collect conversation/reasoning/tool history, expanded template/skill bodies, command descriptions/source paths, system prompts, or context files for classification. Text pasted into a submitted task (or supplied by an earlier input-transforming extension) is still task text and may be sent; there is no reliable secret-redaction guarantee. Auto Setup research/refinement stays on its original model/effort without independent routing. Per-turn and automatic-child routing are [unsupported on the tested host](compatibility.md#per-turn-routing-is-not-supported); there is no YAML flag that enables them.
