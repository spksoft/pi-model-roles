...
for (const selectorContext of ["prompt", "conversation"] as const)
  test(`real SDK uses the selected smaller context model before pre-prompt compaction (${selectorContext})`, async () => {
    ...
    try {
      ...
      for (let turn = 0; turn < 8; turn++)
        await h.session.prompt("Synthetic history. ".repeat(3000));
      const store = new ConfigStore(h.dir);
      const snap = await store.load();
      assert.ok(snap);
      await store.save({ ...config(), selectorContext }, snap.revision);
