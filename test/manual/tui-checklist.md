...
- [ ] In a disposable primary TUI session, open **Auto Setup**, choose one to eight cached models with keyboard-only Space/arrow or j/k/Enter controls, and verify Escape makes no Auto Setup request or role-file change. With more than eight candidates, verify one page fits the picker, PgUp/PgDn and H/L change pages, and `/` or Ctrl+F fuzzy-searches; Escape clears the search before it cancels. Try long/non-English model IDs at 40, 80, and 120 columns in light and dark themes.
