...
- [ ] `/model-roles` completion offers Settings, the currently relevant Enable/Disable Auto Selector action, Use entries for configured roles, Run command with model routing, Context, Context-session, Selector, and Why. Removed status/reload/pause/auto/auto-setup command paths show bounded usage guidance.
...
- [ ] Verify the optional native example only in a normal parent with compatible pi-subagents enabled; confirm registration and synthetic completion/model/effort forwarding. Run the native integration command from that parent without changing inherited safety flags. A child-context verification attempt must fail fast, not skip, retry via another runner, or wait 30 seconds for an absent owner. Record fake-provider results as protocol coverage, not live quality/cost evidence.
...
## Independent selector and context controls

- [ ] `/model-roles selector`: choose/cancel/reset with a slash-containing exact ID; review exact provider, optional effort/default, effective/global context, user-wide save, data exposure, charges and failure behavior. Verify current execution pair/Pi defaults do not change. Unpinned other APIs offer adapter-default effort only; native Responses supports verified levels without clamping. Scoped thinking pins must not offer/accept adapter default and require matching verified effort. Change scope during confirmation and verify no invalid save; configured-provider streamSimple explicit effort must fall back without transmission.
- [ ] Revoke synthetic model availability during confirmation; the profile must not save. Test conflict/locked storage and committed-but-UI-failed warning in a disposable environment; never force overwrite or retry a committed save blindly.
- [ ] Set `context-session prompt` with conversation global and unwritable YAML. Confirm immediate effective prompt-only with no override file/session-entry write. Reject then accept conversation/inherit consent; verify all same-logical-session hosts in this process share the choice and previously sent context cannot be recalled.
- [ ] Verify override survives reload, tree, manual pause/resume, unrelated role/profile edits and every Auto Setup merge. Switch/revisit restores it; new/forked sessions and process restart use global. Verify global context edits do not silently remove a session override. Check same-key change-then-revert invalidates pending decisions.
- [ ] `/why`: distinguish projection categories/partial observations/message-byte clipping from selector-budget removal, unknown summary freshness and unknown provider delivery. Synthetic excluded payload sentinels never appear. Confirm pressure is labelled approximate/unknown, not a capacity guarantee, and no compaction setting is changed.
- [ ] Run `npm run evaluate:demo` without provider credentials. It must identify a harness/contract check, not semantic benchmark evidence. Do not execute a live adapter without separate approval; use the [future evidence checklist](../../docs/selector-evaluation.md#future-paired-evidence-checklist).
