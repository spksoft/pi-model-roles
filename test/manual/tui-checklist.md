...
- [ ] `/model-roles` completion offers Settings, the currently relevant Enable/Disable Auto Selector action, Use entries for configured roles, and Run command with model routing. Removed status/reload/pause/auto/auto-setup command paths show bounded usage guidance.
...
- [ ] Read first-role consent: it must mention explicit `/model-roles run` costs and say task text and role descriptions, no automatic history collection, no tool-loop rerouting, and warn that pasted task text is not secret-filtered. Verify synthetic skill expansion and file-tool history never appear in a later selector request.
