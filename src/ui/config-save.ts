...
export async function confirmFirstCustomRoleRouting(
  ...
  if (effectiveContext === "full")
    historyDisclosure =
      "For ordinary idle input (not the command wrapper), full mode sends up to 100,000 UTF-8 bytes of newest retained user/assistant text and summaries, including raw tool calls/results. Raw thinking, image bytes, custom entries/messages, and !! shell commands are excluded; retained skill text may be included. ";
