...
- Map the user's likely whole-task intents before assigning models. Explore each selected model with each of its supported efforts as a candidate pair (including when only one model is selected); the same model may serve several distinct roles at different efforts. Compare pairs for task fit, then cover as many genuinely distinct, observable intents as the evidence and configured selector can reliably separate. Do not target a fixed role count, assign one role per model or pair, or add redundant roles just for coverage. Leave ambiguous or unsupported work to default.
...
Recommend zero or more roles with justified model/effort settings. It may conclude that no new role is useful. Success is broad useful coverage through evidence-grounded, non-overlapping whole-task roles the user can review, not a role for every pair or a universal winner.
