...
- Write each description as one compact sentence beginning with "Use when" whose selection criteria can be judged from the user's new task text alone in prompt mode, or the task plus bounded retained dialogue in conversation mode; never require hidden facts. Name observable task traits such as scope, uncertainty, risk, required modality, or need for cross-file reasoning; include a concise exclusion when a likely near-miss would otherwise overlap.
...
- Preserve enabled and selectorTimeoutMs, and preserve selectorContext. These are not model-quality tuning knobs. Auto Setup cannot opt users into conversation sharing. Omit default unless a deliberate replacement is justified: default handles unmatched/ambiguous work and also runs the selector, so consider both general-task reliability and selector overhead. No proposal can configure tools, permissions, providers, compaction, context windows, system prompts, temperature, or per-turn switching.`;
