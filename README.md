...
Auto Setup compares each selected model at its supported reasoning efforts against your existing roles and settings. One model can serve several distinct roles at different efforts—even if it is the only candidate. The goal is useful task coverage, not one role per model or a fixed role count; ambiguous tasks fall back to `default`. Without stated priorities, its guidance balances reliability, latency, and cost; recommendations are not measured guarantees. See [how recommendations are designed](docs/configuration.md#how-auto-setup-designs-recommendations).
